...
public sealed class DirectTopicEndpoints : IEndpoint
{
    ...
    private static async Task<IResult> GetAvailableMentors(Guid groupId, ISender sender, CancellationToken ct)
        => Ok(await sender.Send(new GetAvailableMentorsQuery(groupId), ct));

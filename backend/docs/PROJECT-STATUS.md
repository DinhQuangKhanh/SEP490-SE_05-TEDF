...
Endpoint counts are HTTP endpoints under `TEDF.API/Endpoints/`; "App files" is the rough size of the Application feature (`Features/<Context>`). The endpoint folders are **feature-based**, so a few Application contexts (`Mentor`, `Departments`, `Dashboard`) no longer have a same-named endpoint folder — their routes are distributed across the feature folders noted below.
...
| Departments        | 11        | — (distributed)          | ✅     | Routes live in `Projects` (dept projects), `Evaluations` (evaluators/assign/final-decision), `Users` (assign-head), `Majors` |
...
| Mentor             | 7         | — (distributed)          | ✅     | Application feature intact; endpoints split into `Topics` (`/api/topics/mentor`), `TopicPools` (topic update/resubmit), `Dashboard`, `Groups` (`/api/groups/mentor`) |

...
- **Transactional email is live (Firestore Trigger Email):** twelve flows send mail — roster publish (students + lecturers), topic proposed to the department head, evaluator assigned, evaluator finished, evaluator consensus, the department head's final decision, group invitation, group join request, group join decision, and support tickets (created / replied / resolved). Handlers compose one message per recipient and hand them to Hangfire; `FirestoreMailQueue` writes them to the `mail` collection under a deterministic document id, which is what makes delivery exactly-once across retries. Group and support mail is gated by the `EmailOnGroupMembership` / `EmailOnSupportTicket` admin settings.
...
| Admin "send test email"          | ✅     | `FirestoreEmailSender` writes an inline `message` document to the same `mail` collection, so the test exercises the real delivery path. The MailKit/SMTP implementation was removed |

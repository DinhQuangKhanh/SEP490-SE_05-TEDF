...
public class CreateGroupCommandHandler : ICommandHandler<CreateGroupCommand, Guid>
{
    ...
    public async Task<Guid> Handle(CreateGroupCommand request, CancellationToken cancellationToken)
    {
        ...
        // Check if student is already in an active group next semester
        ...
        // Create group with current student as leader (max members from system settings)
        var maxMembers = await _settings.GetIntAsync(SettingKeys.MaxGroupMembers, 5, cancellationToken);
        var group = Group.Create(code, nextSemester.Id, studentId, request.Name, maxMembers);

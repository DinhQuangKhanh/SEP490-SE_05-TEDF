...
namespace TEDF.Infrastructure.EventHandlers.TopicPool
{
    ...
    public class TopicRegistrationConfirmedEventHandler : INotificationHandler<TopicRegistrationConfirmedEvent>
    {
        ...
        public async Task Handle(TopicRegistrationConfirmedEvent notification, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Topic registration confirmed: {RegistrationId}, Project: {ProjectId}",
            ...
            if (studentIds.Count > 0)
            {
                await _notificationService.SendToMultipleAsync(
                    ...
                    $"Giảng viên đã chấp nhận đăng ký đề tài \"{projectName}\".",

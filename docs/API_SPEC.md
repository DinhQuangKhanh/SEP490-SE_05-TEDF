...
These contexts are scaffolded but not exposed over HTTP yet (see `backend/docs/PROJECT-STATUS.md`):

- **Reports** (`/api/reports`) — PDF/Excel export not implemented.
- **Chats** (`/api/chats`) — no REST endpoints; only the SignalR hub + storage exist.
- **Meetings** (`/api/meetings`) — not implemented.

          ...
          <motion.div variants={item} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            {loading ? (
              ...
              <>
                ...
                <div className="bg-white border-t border-slate-200 px-6 py-4 flex items-center justify-between">
                  <p className="text-sm text-slate-500">
                    Hiển thị <span className="font-medium text-slate-900">{(currentPage - 1) * PAGE_SIZE + 1}</span> đến{" "}
                    <span className="font-medium text-slate-900">{Math.min(currentPage * PAGE_SIZE, totalCount)}</span>{" "}
                    trong số <span className="font-medium text-slate-900">{totalCount}</span> đề tài
                  </p>
                  {totalPages > 1 && (
                    <div className="flex items-center gap-1">
                      <button
          ...
          {loading ? (
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <InfoField label="Học kỳ" value={project.semesterName} icon="calendar_today" />

    ...
    <div className={`relative px-2 pt-6 pb-2 ${className}`}>
      ...
      <div className="relative z-10 flex justify-between w-full">
        {phases.map((phase, idx) => {
          ...
          return (
            <div key={idx} className={`flex flex-col items-center gap-2 ${isPending ? "opacity-60" : ""}`}>
              {/* Icon circle */}
              <div className="relative flex items-center justify-center">
                {isCurrent && <div className="absolute inset-0 rounded-full bg-green-500/40 animate-ping" />}
                <div

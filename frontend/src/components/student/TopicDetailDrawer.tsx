      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 bottom-0 right-0 z-50 flex flex-col w-full max-w-lg bg-white shadow-2xl"
          >
            {/* Header */}
              ...
              <div className="min-w-0">
                {detail && (
                  <div className="flex items-center gap-2 mb-2">
                    <span
                      className={`${chipColor} px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border`}
                    >
                      {detail.majorCode}
                    </span>
            ...
            <div className="flex-1 p-6 overflow-y-auto scrollbar-hide">
              ...
              {error && (
                <div className="p-4 text-sm text-red-700 border border-red-200 rounded-lg bg-red-50">
                  <span className="mr-1 text-base align-middle material-symbols-outlined">error</span>
                  {error}
                </div>
              )}
              ...
              {detail && !loading && (
                <div className="flex flex-col gap-5">
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-400">
                    <span>
                      Mã: <span className="font-mono text-slate-600">{detail.code}</span>
                    </span>
                    <span>•</span>
                    <span>
                      Tối đa: <span className="font-semibold text-slate-600">{detail.maxStudents} SV</span>
                    </span>
                  </div>
                  ...
                  {detail.objectives && (
                    <Section title="Mục tiêu" icon="flag">
                      <p className="text-sm leading-relaxed whitespace-pre-line text-slate-600">{detail.objectives}</p>
                    </Section>
                  )}
                  ...
                  {detail.scope && (
                    <Section title="Phạm vi & Yêu cầu" icon="tune">
                      <p className="text-sm leading-relaxed whitespace-pre-line text-slate-600">{detail.scope}</p>
                    </Section>
                  )}
                      ...
                      <div className="flex flex-col gap-2">
                        {detail.mentors.map((m) => (
                          <div key={m.mentorId} className="flex items-center gap-3">
                            <div
                              className={`w-8 h-8 rounded-full ${getInitialsColor(m.fullName)} flex items-center justify-center text-xs font-bold`}
                            >
              ...
              <div className="flex gap-3 px-6 py-4 border-t border-slate-100 shrink-0">
                ...
                {!hasGroup ? (
                  <button
                    ...
                    disabled
                  >
                    <span className="text-lg material-symbols-outlined">group_add</span>
                    Tham gia nhóm trước
                  </button>
                ) : groupHasProject ? (
                  <button
                    ...
                    disabled
                  >
                    <span className="text-lg material-symbols-outlined">block</span>
                    Nhóm đã có đề tài
                  </button>
                ) : !isLeader ? (
                  <button
                    ...
                    disabled
                  >
                    <span className="text-lg material-symbols-outlined">lock</span>
                  ...
                  >
                    <span className="text-lg material-symbols-outlined">app_registration</span>
                    Đăng ký đề tài
                  </button>
                ) : (
                  <button
...
function Section({ title, icon, children }: { title: string; icon: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        <span className="text-base material-symbols-outlined text-primary">{icon}</span>

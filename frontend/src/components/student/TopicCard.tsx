import { TopicInPoolItem } from "@/types";
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <div className="flex items-center justify-between mb-3">
          <span
            className={`${colors.chip} px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border`}
          >
            {topic.majorCode}
          </span>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
          <div className="flex items-center min-w-0 gap-2">
            <div
              className={`w-7 h-7 rounded-full ${getInitialsColor(topic.mentorName)} flex items-center justify-center text-[10px] font-bold shrink-0`}
            >
              {getInitials(topic.mentorName)}
            </div>
            <span className="text-xs font-medium truncate text-slate-600">{topic.mentorName}</span>
          </div>
          <span className="text-[10px] text-slate-400 shrink-0 ml-2">Tối đa: {topic.maxStudents} SV</span>
        </div>
      </div>
      ...
      <div className="px-5 py-3 bg-slate-50/50 border-t border-[#e9ecf1] flex gap-2">
        ...
        >
          <span className="text-base material-symbols-outlined">visibility</span>
          Chi tiết
        </button>
    ...
    <div className="bg-white rounded-xl border border-[#e9ecf1] shadow-sm overflow-hidden flex flex-col animate-pulse">
      ...
      <div className="flex flex-col flex-1 gap-3 p-5">
        ...
        <div className="flex gap-1.5">
          <div className="h-5 rounded-full w-14 bg-slate-100" />
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <div className="h-3 rounded w-28 bg-slate-200" />
          </div>
          <div className="w-16 h-3 rounded bg-slate-200" />
        </div>
      </div>
      <div className="px-5 py-3 bg-slate-50/50 border-t border-[#e9ecf1] flex gap-2">
        <div className="w-10 rounded-lg h-9 bg-slate-200" />
        <div className="flex-1 rounded-lg h-9 bg-slate-200" />
        <div className="flex-1 rounded-lg h-9 bg-slate-200" />
      </div>
    </div>
  );

    ...
    <Modal onClose={onClose} contentClassName="w-full max-w-lg bg-white shadow-2xl rounded-xl overflow-hidden">
      ...
      <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div>
          ...
          <input className={inputClass} value={form.fullName} onChange={(e) => set("fullName", e.target.value)} placeholder="Nguyễn Văn A" />
        </div>

        <div>
          <label className={labelClass}>Email (@fpt.edu.vn)</label>
          <input className={inputClass} type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="a@fpt.edu.vn" />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={labelClass}>{isStudent ? "Mã số sinh viên" : "Mã giảng viên"}</label>
            <input className={inputClass} value={form.code} onChange={(e) => set("code", e.target.value)} placeholder={isStudent ? "SE150001" : "GV0001"} />
          </div>
          <div>
            <label className={labelClass}>Số điện thoại</label>
            <input className={inputClass} value={form.phone ?? ""} onChange={(e) => set("phone", e.target.value)} placeholder="09xxxxxxxx" />
          </div>
        </div>
        ...
        {isStudent ? (
          ...
          <div>
            ...
            <input className={inputClass} value={form.academicTitle ?? ""} onChange={(e) => set("academicTitle", e.target.value)} placeholder="ThS / TS / PGS.TS" />
          </div>
        )}
      ...
      <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50">
        <button onClick={onClose} className="px-4 py-2 text-sm font-semibold rounded-lg text-slate-600 hover:bg-slate-100">Hủy</button>
        <button
